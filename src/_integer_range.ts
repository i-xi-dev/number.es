...
export abstract class _IntegerRangeBase<T extends (number | bigint)>
  implements IntegerRange<T> {
  readonly #min: T;
  readonly #max: T;
...
export namespace _IntegerRange {
  export function parse<T extends (number | bigint)>(
    rangeLike: IntegerRange.Like<T>,
    validator: (test: unknown) => test is T,
  ): IntegerRange.Struct<T> {
